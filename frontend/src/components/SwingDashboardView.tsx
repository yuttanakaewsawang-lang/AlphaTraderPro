...
import { Wallet, LineChart, TrendingUp, TrendingDown, Zap, Target, Waves } from 'lucide-react';
import api from '../api';
import type { LiveDecision, SwingStatusResponse } from '../types/strategy';

// Dashboard เฉพาะกลยุทธ์ Swing Trade (EA3 — trend pullback) — แยกไฟล์ตาม pattern เดียวกับ
// SniperDashboardView (self-contained ไม่ปนเงื่อนไข engine ใน DashboardView ของ SMC)
// โทนสีประจำกลยุทธ์ = ฟ้าอมเขียว #40C8E0 (ตรงการ์ดหน้าเลือกกลยุทธ์)

const SWING_CYAN = '#40C8E0';
...
}> = ({ label, value, sub, icon: Icon, iconColor = SWING_CYAN, iconBg = 'rgba(64,200,224,0.15)' }) => (
...
const Arrow: React.FC<{ active: boolean; color?: string }> = ({ active, color = SWING_CYAN }) => (
...
const SwingDashboardView: React.FC<{ symbol: string }> = ({ symbol }) => {
  const [online, setOnline] = useState(false);
  const [account, setAccount] = useState<AccountInfo | null>(null);
  const [status, setStatus] = useState<SwingStatusResponse | null>(null);
    ...
    const fetchAll = async () => {
      try {
        const [accRes, stRes, posRes, decRes] = await Promise.all([
          ...
          api.get<SwingStatusResponse>('/api/swing/status', { params: { symbol } }),
  ...
  const priceAboveEma = !!setup && setup.price != null && setup.price > setup.ema;
    ...
    <div className="ios-fade-in flex flex-col gap-3 h-full">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold"
          style={{ color: SWING_CYAN, background: 'rgba(64,200,224,0.12)', border: '1px solid rgba(64,200,224,0.30)' }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 shrink-0">
        ...
        <KPI label={`EMA${setup?.pullback_ema ?? cfg?.pullback_ema ?? ''} Pullback`}
          icon={Waves}
          iconColor={setup ? SWING_CYAN : 'rgba(235,235,245,0.3)'}
          iconBg={setup ? 'rgba(64,200,224,0.12)' : 'rgba(255,255,255,0.06)'}
          value={
            <span style={{ color: setup ? '#FFFFFF' : 'rgba(235,235,245,0.40)' }}>
              {setup ? setup.ema.toFixed(2) : 'รอข้อมูล'}
            </span>
          }
          sub={setup ? `${tf} · ราคา${priceAboveEma ? 'เหนือ' : 'ใต้'} EMA` : undefined} />
      </div>
      ...
      <div className="lux-card px-4 py-3 shrink-0">
        <p className="lux-title mb-2.5">Live Pipeline — Swing Pullback</p>
        <div className="flex items-stretch gap-1">
          <PipeNode
            color={SWING_CYAN} title="Trend Scan"
            value={running ? `RUNNING · ${tf}` : 'STOPPED'}
            sub={trendOn ? `เทรนด์ ${biasText(bias)} (${cfg?.trend_filter_mode === 1 ? 'HH/HL' : 'EMA50'})` : 'trend filter ปิด'}
            state={running ? (bias !== 0 ? 'active' : 'idle') : 'idle'}
          />
          <Arrow active={running && bias !== 0} />
          <PipeNode
            color={SWING_CYAN} title="Pullback to EMA"
            value={setup ? (setup.touched ? 'ราคาแตะ EMA แล้ว' : `รอย่อแตะ EMA${setup.pullback_ema}`) : 'รอข้อมูล'}
            sub={setup && setup.price != null ? `ราคา ${setup.price.toFixed(2)} · EMA ${setup.ema.toFixed(2)}` : '—'}
            state={setup?.touched ? 'active' : 'idle'}
            flash={flash.scan}
          />
          <Arrow active={!!setup?.touched} />
          <PipeNode
            color={SWING_CYAN} title="Confirm Candle"
            value="รอแท่งปิดกลับทิศเทรนด์"
            sub={status?.last_message ?? '—'}
            state={running && setup?.touched ? 'active' : 'idle'}
          />
          <Arrow active={positions.length > 0} />
          <PipeNode
            color={SWING_CYAN} title="Execution"
          ...
          <Arrow active={positions.length > 0} />
          <PipeNode
            color={SWING_CYAN} title="SL/TP @ Order"
            value={`RR 1:${cfg?.rr ?? '-'}`}
            sub={positions.length > 0 ? `โบรกเกอร์ดูแล ${positions.length} ไม้ (SL ใต้ swing)` : 'SL/TP ตั้งที่ออเดอร์ตั้งแต่เปิดไม้'}
            state={positions.length > 0 ? 'active' : 'idle'}
      ...
      {/* Setup panel + Live log */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 flex-1 min-h-0">
        <div className="lux-card p-4 min-h-0 flex flex-col">
          <p className="lux-title mb-2 shrink-0">Swing Setup — เทรนด์ + EMA{setup?.pullback_ema ?? ''} ({tf})</p>
          {!setup ? (
            ...
            <div className="flex-1 overflow-y-auto min-h-0 space-y-4">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs tabular-nums">
                ...
                <div className="flex justify-between">
                  <span className="text-ink-muted">EMA{setup.pullback_ema}</span>
                  <span className="text-ink">{setup.ema.toFixed(2)}</span>
                </div>
                {setup.price != null && (
                  <div className="flex justify-between">
                ...
                {setup.price != null && (
                  <div className="flex justify-between">
                    <span className="text-ink-muted">ห่างจาก EMA</span>
                    <span className="text-ink">{(setup.price - setup.ema).toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-ink-muted">แตะ EMA ในแท่งล่าสุด</span>
                  <span className={setup.touched ? 'text-emerald-400 font-semibold' : 'text-ink-muted'}>
                    {setup.touched ? 'แตะแล้ว' : 'ยังไม่แตะ'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">RR เป้าหมาย</span>
                  <span className="text-ink">1:{cfg?.rr ?? '-'}</span>
                </div>
              </div>
              <p className="text-[10px] text-ink-faint leading-relaxed">
                สัญญาณเข้า = เทรนด์ TF ใหญ่ชัด (HH/HL) → ราคาย่อมาแตะ EMA{setup.pullback_ema} →
                แท่งปิดกลับทิศเดียวกับเทรนด์ · SL ใต้/เหนือ swing {cfg?.sl_lookback_bars ?? 10} แท่งล่าสุด + buffer ·
                TP = {cfg?.rr ?? 3}× ระยะ SL — แนะนำทุน $1,000 ขึ้นไป (ทุนเล็ก SL กว้างจะโดน Min-lot Risk Guard ข้ามไม้)
              </p>
            </div>
          )}
        </div>
...
export default SwingDashboardView;
