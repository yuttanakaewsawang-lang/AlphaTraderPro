"""
เทียบ Trend Filter เปิด (mode=1 HH/HL) vs ปิด — ใช้ config ปัจจุบันตามภาพจริง:
Retest Zone=ON, OB Entry=ON, FVG Entry=OFF, Engulfing Confirm=OFF,
buffer_atr=0.15, min_sl_atr=0.5 (ค่าที่แนะนำแทน 1.2), use_swing_sl=1, zone M5/M5
tick-mode 4 เดือน (fill จริง)
"""
import sys, os
...
CURRENT_CONFIG = {
    "zone_timeframe": "M5", "entry_timeframe": "M5",
    "zone_expiry_bars": 50, "zone_atr_mult": 0.3,
    "min_candle_atr": 0.3, "max_candle_atr": 2.5,
    "buffer_atr": 0.15, "min_sl_atr": 0.5, "max_ob_zone_atr": 5.0,
    "use_swing_sl": 1,
    "require_retest": 1, "enable_ob_entry": 1, "enable_fvg_entry": 0,
    "require_engulfing": 0,
    "trend_filter_mode": 1,
}
...
def main():
    ...
    print(f"\n{'='*96}")
    print(f"Trend Filter ON/OFF - {SYMBOL} M5  ({months[0]} to {months[-1]})  [tick-mode, real fills]")
    print(f"{'='*96}")

    variants = [
        ("Trend Filter ON (mode=1 HH/HL)", {"use_trend_filter": 1}),
        ("Trend Filter OFF",               {"use_trend_filter": 0}),
    ]
